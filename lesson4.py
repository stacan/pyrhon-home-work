...
n = '731671765313306249192251196744265747423553491'\
'94934969835203127745063262395783180169848018694788'\
'51843858615607891129494954595017379583319528532088'\
'05511125406987471585238630507156932909632952274430'\
'4355766896648950445244523161731856403098711121722383113'\
'62229893423380308135336276614282806444486645238749'\
'30358907296290491560440772390713810515859307960866'\
'70172427121883998797908792274921901699720888093776'\
'65727333001053367881220235421809751254540594752243'\
...
for i in range(1,len(n)):
    if int(n[i]) - int(n[i-1]) == 1:
        count += 1
        if count >= 5:
            pr = int(n[i-4])*int(n[i-3])*int(n[i-2])*int(n[i-1])*int(n[i])
            if pr > max_pr:
                max_pr = pr
                index = i-4
    if int(n[i]) - int(n[i-1]) != 1:
...
print(max_pr)
print(index)
print(n[index], n[index+1], n[index+2], n[index+3], n[index+4])
